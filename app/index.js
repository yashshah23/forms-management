...
app.controller('formsAddController', function ($scope, $rootScope, $controller, $http, R, S, $location, $q, $timeout) {
    ...
    $scope.addQuestion = function () {
        ...
        $scope.addField($scope.defaultAdd.title, $scope.defaultAdd.field_type.id, $scope.data1.is_required, $scope.data1.default_value, $scope.data1.is_multiple, 0)
    ...
    $scope.addField = function (field, fieldType, is_required, default_value, is_multiple, isCreated) {
        ...
        if (field && fieldType) {
            //test67 #remember 
        	if((fieldType == 3 || fieldType == 4 || fieldType == 5) && ($scope.fieldSources[field] == undefined || $scope.fieldSources[field].length == 0) && isCreated == 1) {
...
app.controller('formsEditController', function ($scope, $routeParams, R, $controller, S, H, $rootScope, $q, $timeout, $location, $http) {
  ...
  // Use toUpperCase() to ignore character casing
		const genreA = Number(a.seq);
		const genreB = Number(b.seq);
    ...
    $scope.load = function () {
        ...
        R.get('forms/' + $scope.id).get(function (res) {
            $scope.data = res;
            $scope.data.categoryId = res.category.id;
            ...
            R.get('users').query({}, function (records) {
	            for (let x in $scope.userid) {
	                ...
	                // $timeout(function () {
	                $scope.selectedPeopleTemp.push(records.filter(function (obj) { return obj.id == $scope.userid[x] }));
	                // }, 100);
	            }
	        ...
	        R.get('user_groups').query({ status: 1 }, function (res) {
	            ...
	            for (let x in $scope.groupid) {
	                ...
	                $scope.selectedGroupTemp.push(res.filter(function (obj) { return obj.id == $scope.groupid[x] }));
	            }
	            $scope.selectedGroup = $scope.selectedGroupTemp.filter(function (el) {
					return el.length != 0;
				});
	        });
            ...
            R.get('form_formulas').query({}, function (d) {
                ...
                for (let x in data) {
                    ...
                    $scope.fields.push({
                        ...
                        field_type: data[x].field.field_type,
    ...
    $scope.addQuestion = function () {
        ...
        $scope.addField($scope.defaultEdit.title, $scope.defaultEdit.field_type.id, $scope.data1.is_required, $scope.data1.default_value, $scope.data1.is_multiple, 0);
    ...
    function updateFields(k, existingFields) {
    	if(k >= existingFields.length) {
    		
    	} else {
    		var data = {
    			id: existingFields[k].id,
    			seq: existingFields[k].seq
    		}
    		$http.put(H.SETTINGS.baseUrl + '/form_fields', data).then(function(response) {
    			updateFields(k + 1, existingFields);	
    		});
    	}
    }
    
    function deleteFields(k, deleteIds) {
    	if(k >= deleteIds.length) {
    		
    	} else {
    		$http.delete(H.SETTINGS.baseUrl + '/form_fields/' + deleteIds[k]).then(function(response) {
    			deleteFields(k + 1, deleteIds);
        	});
    	}
    }
    ...
    $scope.save = function () {
		...
		for(var i = 0; i < $scope.fields.length; i++) {
			if($scope.fields[i].id != undefined) {
				existingFields.push($scope.fields[i]);
				existingFields[j].seq = i;
				j++;
			} else {
        ...
        for (var i in newFields) {
            var field = new Field();
            field.title = newFields[i].title;
            field.field_type_id = newFields[i].field_type_id;
            requiredFields[newFields[i].title] = {
                required: newFields[i].is_required,
                default_value: newFields[i].default_value,
                is_multiple: newFields[i].is_multiple,
                is_formula: newFields[i].formula ? 1 : 0,
                seq: newFields[i].seq
            };
            fieldSavePromises.push(field.$save());
        }
        $q.all(fieldSavePromises).then(function (r) {
            ...
            $q.all(form_fields).then(function (r) {
                ...
                R.get('form_fields').query({
                    ...
                    $q.all(form_field_ds).then(function (r) {           	
    					deleteFields(0, $scope.deleteIds);
                        $scope.showFormSavedModal();
                    })
                }, function (e) {
                    // $scope.launchErrorModal();
    ...
    $scope.deleteField = function (obj, index) {
        if(obj.id != undefined) {
...
app.controller('formsRecordsController', function ($scope, $rootScope, $controller, $http, R, S, $location, $q, $routeParams, H, $timeout) {
	...
	R.get('form_fields').query({
		...
		$scope.data.form_fields = r;
		$scope.data.form_fields.sort(compare);
	}, function (e) {
		console.log(e);
	});
...
app.controller('formsCategoryAddController', function($scope, $rootScope, $http, R, S, $location, $q) {
	...
	$scope.showErrorModalNewEntrytitle = function() {
        $scope.modalOptions.open($scope.errorModalOptionsNewEntrytittle);
    }
    
    $scope.errorModalOptionsNewEntrytittle = {
    	header: 'Warning ...',
	...
	{
		if(e.status ==409){
			$scope.showErrorModalNewEntrytitle();
			//$scope.data.title = '';
...
app.controller('formsProcedureAddController', function($scope, $http, R, S, $location, $q) {
    ...
    $scope.errorModalOptionsNewEntry = {
    	header: 'Warning ...',
        text: 'Procedure name already exist !!',
        showOk: true,
        okText: 'Ok',
        onOkClick: function () {
...
app.controller('formsQuestionBankAddController', function ($scope, $rootScope, $http, R, S, $location, $q) {
	$scope.selectedPerson = "";
	$scope.selectedPeople = [];
	$scope.fields = [];
	$scope.isDisabled = false;
...
/*global app*/
app.controller('homeController', function ($scope, $rootScope, H, R, $http, S) {
